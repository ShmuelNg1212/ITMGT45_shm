...
def tic_tac_toe(board):
    ...
    rev_diag = [board[i] [size - 1 - i] for i in range(size)]
    if len(set(rev_diag)) == 1 and rev_diag[0] != "":
        return rev_diag[0]
...
def eta(first_stop, second_stop, route_map):
    ...
    while current_stop != second_stop:
        ...
        if not found_leg:
            raise ValueError(f"No outgoing leg from stop {current_stop}")
        
        if current_stop in visited:
            raise ValueError("Route is invalid or does not reach destination")
